...
skip_if_gtid() {
GTID=`mysql -h 127.0.0.1 --port=$S3P -e "show replica status\G" | grep Auto_Position | awk {'print $2'}`
  if [ "a$GTID" = "a1" ]; then
    echo "$1 [Skip]"
    exit 0
  fi
...
check_sql_yes() {
SQL_STATUS=`mysql -h127.0.0.1 --port=$2 -e "show replica status\G" | grep Slave_SQL_Running: | awk '{print $2}'`
...
check_master() {
MASTER_PORT=`mysql -h127.0.0.1 --port=$2 -e "show replica status\G" | grep Master_Port: | awk '{print $2}'`
...
get_binlog_file() {
B_FILE=`mysql -h127.0.0.1 --port=$1 -e "show binary log status\G" | grep File | awk {'print $2'}`
  echo $B_FILE
}


get_binlog_position() {
POS=`mysql -h127.0.0.1 --port=$1 -e "show binary log status\G" | grep Position | awk {'print $2'}`
  echo $POS
}
