...
sub connect_and_get_status {
  ...
  if ( $sstatus == 1 ) {
    ...
    my $master_ip = MHA::NodeUtil::get_ip( $status{Source_Host} );
    unless ($master_ip) {
    warn "Master_Host: ", (defined $status{Source_Host} ? $status{Source_Host} : 'undef'), "\n";
    warn "Hostinfo: ", (defined $self->get_hostinfo() ? $self->get_hostinfo() : 'undef'), "\n";
      $log->error(
        sprintf(
" Failed to get an IP address of %s! %s replicates from %s, but maybe invalid.",
          $status{Source_Host}, $self->get_hostinfo(), $status{Source_Host}
        )
      );
      croak;
    }
    $self->{Master_IP}     = $master_ip;
    $self->{Master_Port}   = $status{Source_Port};
    $self->{not_slave}     = 0;
    $self->{Master_Host}   = $status{Source_Host};
    $self->{repl_user}     = $status{Source_User} unless ( $self->{repl_user} );
...
sub current_slave_position {
  ...
  $self->{Master_Log_File}       = $status{Source_Log_File};
  $self->{Read_Master_Log_Pos}   = $status{Read_Source_Log_Pos};
  $self->{Relay_Master_Log_File} = $status{Relay_Source_Log_File};
  $self->{Exec_Master_Log_Pos}   = $status{Exec_Source_Log_Pos};
...
sub has_replication_problem {
  ...
  elsif ( $status{Replica_IO_Running} ne "Yes" ) {
    $log->error(
      sprintf( "Slave IO thread is not running on %s", $self->get_hostinfo() )
    );
    return 2;
  }
  elsif ( $status{Replica_SQL_Running} ne "Yes" ) {
    $log->error(
      sprintf( "Slave SQL thread is not running on %s", $self->get_hostinfo() )
    );
    return 3;
  }
  elsif ( $status{Seconds_Behind_Source}
    && $status{Seconds_Behind_Source} > $allow_delay_seconds )
  {
    $log->error(
      sprintf(
  ...
  elsif ( !defined( $status{Seconds_Behind_Source} ) ) {
    $log->error(
...
sub reset_slave_all {
  ...
  $dbhelper->reset_replica_source_host();
}
...
sub wait_until_slave_starts($$) {
  ...
  for ( my $i = 0 ; $i < $retry_count ; $i++ ) {
    ...
    if ( $type eq "IO" ) {
      return 0 if ( $status{Replica_IO_Running} eq "Yes" );
    }
    elsif ( $type eq "SQL" ) {
      return 0 if ( $status{Replica_SQL_Running} eq "Yes" );
    }
    else {
      return 0
        if ( $status{Replica_IO_Running} eq "Yes"
        && $status{Replica_SQL_Running} eq "Yes" );
    }

    if ( $status{Replica_SQL_Running} eq "No" && $status{Last_Errno} ne '0' ) {
      $log->error(
...
sub wait_until_slave_stops {
  ...
  for ( my $i = 0 ; $i < $retry_count ; $i++ ) {
    ...
    if ( $type eq "IO" ) {
      return 0 if ( $status{Replica_IO_Running} eq "No" );
    }
    elsif ( $type eq "SQL" ) {
      return 0 if ( $status{Replica_SQL_Running} eq "No" );
    }
    else {
      return 0
        if ( $status{Replica_IO_Running} eq "No"
        && $status{Replica_SQL_Running} eq "No" );
    }
    sleep($interval);
  }
  $log->error(
...
sub is_sql_thread_error {
  ...
  return 0 if ( $status{Replica_SQL_Running} eq "Yes" );
  if ( $status{Replica_SQL_Running} eq "No" && $status{Last_Errno} eq '0' ) {
    $log->warning(
