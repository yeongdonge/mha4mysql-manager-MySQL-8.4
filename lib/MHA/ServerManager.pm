...
sub validate_repl_filter($$) {
  ...
  foreach (@slaves) {
      ...
      || $replicate_wild_ignore_table ne $_->{Replicate_Wild_Ignore_Table} )
    {
      $log->error(
        sprintf(
"Replication filtering check failed on %s! All slaves must have same replication filtering rules. Check SHOW REPLICA STATUS output and set my.cnf correctly.",
          $_->get_hostinfo() )
      );
    ...
    if ( $_->{log_bin} ) {
        ...
        || $binlog_ignore_db ne $_->{Binlog_Ignore_DB} )
      {
        $log->error(
          sprintf(
"Binlog filtering check failed on %s! All log-bin enabled servers must have same binlog filtering rules (same binlog-do-db and binlog-ignore-db). Check SHOW BINARY LOG STATUS output and set my.cnf correctly.",
            $_->get_hostinfo() )
        );
        $self->print_filter_rules($master);
        return 1;
...
sub is_master_reachable_from_slaves($$) {
  ...
  foreach (@slaves) {
    ...
    if ( $status{Status} ne '0' || !defined( $status{Replica_IO_Running} ) ) {
      $log->error(
...
sub read_slave_status($) {
  ...
  foreach (@slaves) {
    ...
    $_->{latest}                = 0;
    $_->{Master_Log_File}       = $status{Source_Log_File};
    $_->{Read_Master_Log_Pos}   = $status{Read_Source_Log_Pos};
    $_->{Relay_Master_Log_File} = $status{Relay_Source_Log_File};
    $_->{Exec_Master_Log_Pos}   = $status{Exec_Source_Log_Pos};
...
sub change_master_and_start_slave {
  ...
  if ( $self->is_gtid_auto_pos_enabled() && !$target->{is_mariadb} ) {
    $dbhelper->change_source_gtid( $addr, $master->{port},
