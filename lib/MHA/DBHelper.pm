...
#use constant Slave_IO_State              => "Slave_IO_State"; -- edtied by DYKIM
use constant Replica_IO_State              => "Replica_IO_State";

#use constant Slave_SQL_Running           => "Slave_SQL_Running"; -- edtied by DYKIM
use constant Replica_SQL_Running           => "Replica_SQL_Running";

#use constant Slave_IO_Running            => "Slave_IO_Running";
use constant Replica_IO_Running            => "Replica_IO_Running";

#use constant Master_Log_File             => "Master_Log_File"; -- edtied by DYKIM
use constant Source_Log_File             => "Source_Log_File";

#use constant Master_Host                 => "Master_Host"; -- edtied by DYKIM
use constant Source_Host                 => "Source_Host";

#use constant Master_User                 => "Master_User"; -- edtied by DYKIM
use constant Source_User                 => "Source_User";

#use constant Master_Port                 => "Master_Port"; -- edtied by DYKIM
use constant Source_Port                 => "Source_Port";
...
#use constant Read_Master_Log_Pos         => "Read_Master_Log_Pos"; -- edtied by DYKIM
use constant Read_Source_Log_Pos         => "Read_Source_Log_Pos";

#use constant Relay_Master_Log_File       => "Relay_Master_Log_File"; -- edtied by DYKIM
use constant Relay_Source_Log_File       => "Relay_Source_Log_File";

#use constant Exec_Master_Log_Pos         => "Exec_Master_Log_Pos"; -- edtied by DYKIM
use constant Exec_Source_Log_Pos         => "Exec_Source_Log_Pos";

use constant Relay_Log_File              => "Relay_Log_File";
use constant Relay_Log_Pos               => "Relay_Log_Pos";

#use constant Seconds_Behind_Master       => "Seconds_Behind_Master"; -- edtied by DYKIM
use constant Seconds_Behind_Source       => "Seconds_Behind_Source";
...
#"CHANGE REPLICATION SOURCE TO='%s', SOURCE_PORT=%d, SOURCE_USER='%s', SOURCE_PASSWORD='%s', SOURCE_LOG_FILE='%s', SOURCE_LOG_POS=%d";
use constant Change_Source_SQL =>
"CHANGE REPLICATION SOURCE TO SOURCE_HOST='%s', SOURCE_PORT=%d, SOURCE_USER='%s', SOURCE_PASSWORD='%s', SOURCE_LOG_FILE='%s', SOURCE_LOG_POS=%d, SOURCE_SSL=1";

#use constant Change_Master_NoPass_SQL =>
#"CHANGE REPLICATION SOURCE TO='%s', SOURCE_PORT=%d, SOURCE_USER='%s', SOURCE_LOG_FILE='%s', SOURCE_LOG_POS=%d";
use constant Change_Source_NoPass_SQL =>
"CHANGE REPLICATION SOURCE TO SOURCE_HOST='%s', SOURCE_PORT=%d, SOURCE_USER='%s', SOURCE_LOG_FILE='%s', SOURCE_LOG_POS=%d, SOURCE_SSL=1";

#use constant Change_Master_Gtid_SQL =>
#"CHANGE REPLICATION SOURCE TO='%s', SOURCE_PORT=%d, SOURCE_USER='%s', SOURCE_PASSWORD='%s', SOURCE_AUTO_POSITION=1";
use constant Change_Source_Gtid_SQL =>
"CHANGE REPLICATION SOURCE TO SOURCE_HOST='%s', SOURCE_PORT=%d, SOURCE_USER='%s', SOURCE_PASSWORD='%s', SOURCE_AUTO_POSITION=1, SOURCE_SSL=1";

#use constant Change_Master_Gtid_NoPass_SQL =>
#"CHANGE REPLICATION SOURCE TO='%s', SOURCER_PORT=%d, SOURCE_USER='%s', SOURCE_AUTO_POSITION=1";
use constant Change_Source_Gtid_NoPass_SQL =>
"CHANGE REPLICATION SOURCE TO SOURCE_HOST='%s', SOURCER_PORT=%d, SOURCE_USER='%s', SOURCE_AUTO_POSITION=1, SOURCE_SSL=1";

#use constant Reset_Slave_Master_Host_SQL => "RESET SLAVE /*!50516 ALL */"; -- edtied by DYKIM
use constant Reset_Replica_Source_Host_SQL => "RESET REPLICA /*!50516 ALL */";

#use constant Reset_Slave_SQL             => "RESET SLAVE"; -- edtied by DYKIM
use constant Reset_Replica_SQL             => "RESET REPLICA";

#use constant Change_Master_Clear_SQL     => "CHANGE MASTER TO MASTER_HOST=''"; -- edtied by DYKIM
use constant Change_Source_Clear_SQL     => "CHANGE REPLICATION SOURCE TO SOURCE_HOST=''";

#use constant Show_Slave_Status_SQL       => "SHOW SLAVE STATUS";
use constant Show_Replica_Status_SQL       => "SHOW REPLICA STATUS";
...
#use constant Show_Master_Status_SQL => "SHOW MASTER STATUS"; -- edtied by DYKIM
use constant Show_Source_Status_SQL => "SHOW BINARY LOG STATUS";

#use constant Stop_IO_Thread_SQL     => "STOP SLAVE IO_THREAD"; -- edtied by DYKIM
use constant Stop_IO_Thread_SQL     => "STOP REPLICA IO_THREAD";

#use constant Start_IO_Thread_SQL    => "START SLAVE IO_THREAD"; -- edtied by DYKIM
use constant Start_IO_Thread_SQL    => "START REPLICA IO_THREAD";

#use constant Start_Slave_SQL        => "START SLAVE"; -- edtied by DYKIM
use constant Start_Replica_SQL        => "START REPLICA";

#use constant Stop_Slave_SQL         => "STOP SLAVE"; -- edtied by DYKIM
use constant Stop_Replica_SQL         => "STOP REPLICA";

#use constant Start_SQL_Thread_SQL   => "START SLAVE SQL_THREAD"; -- edtied by DYKIM
use constant Start_SQL_Thread_SQL   => "START REPLICA SQL_THREAD";

#use constant Stop_SQL_Thread_SQL    => "STOP SLAVE SQL_THREAD"; -- edtied by DYKIM
...
#use constant Get_Num_Workers_SQL =>
  #"SELECT \@\@global.slave_parallel_workers AS Value"; -- edtied by DYKIM
use constant Get_Num_Workers_SQL =>
  "SELECT \@\@global.replica_parallel_workers AS Value";
...
#use constant Master_Pos_Wait_NoTimeout_SQL => -- edtied by DYKIM
#  "SELECT MASTER_POS_WAIT(?,?,0) AS Result";
use constant Source_Pos_Wait_NoTimeout_SQL =>
...
sub reset_replica_source_host($) {
  my $self = shift;
  return $self->execute(Reset_Replica_Source_Host_SQL);
}

sub reset_slave_by_change_master($) {
  my $self = shift;
  return $self->execute(Change_Source_Clear_SQL);
}
...
sub change_master($$$$$$$) {
  ...
  my $source_log_file = shift;
  my $source_log_pos  = shift;
  my $source_user     = shift;
  my $source_password = shift;

  my $query;
  if ($source_password) {
    $query = sprintf( Change_Source_SQL,
      $source_host,     $source_port,     $source_user,
      $source_password, $source_log_file, $source_log_pos );
  }
  else {
    $query = sprintf(
      Change_Source_NoPass_SQL,
      $source_host,     $source_port, $source_user,
...
sub change_source_gtid($$$$$) {
  ...
  my $query;
  if ($source_password) {
    $query = sprintf( Change_Source_Gtid_SQL,
      $source_host, $source_port, $source_user, $source_password );
  }
  else {
    $query = sprintf( Change_Source_Gtid_NoPass_SQL,
      $source_host, $source_port, $source_user );
  }
  return $self->execute($query);
}
...
sub reset_slave($) {
  my $self = shift;
  return $self->execute(Reset_Replica_SQL);
}
...
sub start_slave() {
  my $self = shift;
  return $self->execute(Start_Replica_SQL);
}
...
sub stop_slave() {
  my $self = shift;
  return $self->execute(Stop_Replica_SQL);
}
...
sub check_slave_status {
  ...
  if ( !$status{Source_Host}
    || !$status{Source_Log_File} )
  {
    unless ($allow_dummy) {
...
sub read_all_relay_log {
  ...
  do {
    ...
    elsif ( !$io_thread_should_run && $status{Replica_IO_Running} eq "Yes" ) {
      $status{Status} = 3;
      $status{Errstr} = "Slave IO thread is running! Check master status.";
      return %status;
    }
    elsif ( $status{Replica_SQL_Running} eq "No" ) {
      $status{Status} = 4;
      $status{Errstr} = "SQL thread is not running! Check slave status.";
      return %status;
    }
    elsif ( ( $status{Source_Log_File} eq $status{Relay_Source_Log_File} )
      && ( $status{Read_Source_Log_Pos} == $status{Exec_Source_Log_Pos} ) )
    {
      $status{Status} = 0;
      return %status;
    }
    ...
    if ($io_thread_should_run) {
      if (!$status{Replica_IO_State}
        || $status{Replica_IO_State} !~ m/Waiting for master to send event/ )
      {
        $sql_thread_check = 0;
      }
    }
...
sub master_pos_wait($$$) {
  ...
  my $binlog_pos  = shift;
  my $sth         = $self->{dbh}->prepare(Source_Pos_Wait_NoTimeout_SQL);
